...
fn main(filename: String) -> String {
    ...
    let mut leave_count: u64 = 0;
    let mut leaves: Vec<String> = Vec::new();
    
    for line in reader.lines() {
        let line_as_str = line.expect("Cant read new line");
        if leave_count == 0 {
            //leaves have fixed number
            //For ex. : If level count is 4 then leave count should be 2^4 = 16
            let level_count = line_as_str.parse::<u32>().unwrap();
            leave_count = u64::pow(2,level_count);
            leaves = Vec::with_capacity(leave_count as usize);
        }
        else{
    ...
    while leaves.len() < leave_count as usize && leaves.len() >= 2 {
        leaves.push(leaves[leaves.len()-2].clone());
    }
    
    while leaves.len() > leave_count as usize {
